...
# Title: model 5                                                               #
# Date: 9/29/2021                                                              #
# Description: MNL model estimation - basic model + bird species at risk + 
#              species richness2                                               #
################################################################################
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate") {
  ...
  for ( j in 1:apollo_inputs$J ) {
    ...
    V[[ paste0('choice', j ) ]] = b_tc * get(paste0('tc_', j)) + 
      b_nrsr * get(paste0("nrsr_", j)) + 
      b_nrsr2 * get(paste0("nrsr_", j))^2 +
...
#minimum = 1
deltaMethod(model, "(b_sr + (2*b_sr2*1))/ (-b_tc)")

#25th percentile = 39
deltaMethod(model, "(b_sr + (2*b_sr2*39))/ (-b_tc)")

#50th percentile = 72
deltaMethod(model, "(b_sr + (2*b_sr2*72))/ (-b_tc)")

#75th percentile = 112
deltaMethod(model, "(b_sr + (2*b_sr2*112))/ (-b_tc)")

#maximum = 257
deltaMethod(model, "(b_sr + (2*b_sr2*257))/ (-b_tc)")
