...
# Title: model 5                                                               #
# Date: 3/27/2022                                                              #
# Description: Model site choice - include all birding hotpots and use         #
# wage rate as 1/2 of the income + driving cost as marginal cost not average cost #
################################################################################
...
# Travel costs
df_travel_costs <- read_csv("./data/processed/ab-ebd-travel-costs-marginal-onethird-wagerate.csv")
...
# exclude risk species from total species 
df_nonrisk <- df_spe%>%
  left_join(df_risk)%>%
  mutate(nonrisk_spe = n_species -num_species_endangered)%>%
  select(locality_id,nonrisk_spe)
...
df_trip <- df_pt_rel %>%
  ...
  left_join(df_nonrisk, by = c("locality_id"))%>%
...
# always
write.csv(df4,"data/processed/adjusted_cost_1_3_wagerate_mc.csv")
#-------------------------------------------------------------------------------
# import adjusted cost folder
df_adjusted_cost <- read.csv("data/processed/adjusted_cost_1_3_wagerate_mc.csv")
...
# Add back nonrisk bird species
df_modeling = df_modeling%>%
  left_join(df_nonrisk, by = "locality_id")
...
# create number of nonrisk species in wide format
df_nonri <- df_modeling%>%
  pivot_wider(choice_id, names_from = "alt",
              names_prefix = "nrsr_",
              values_from = "nonrisk_spe")
...
df_apollo = df_choice %>%
  ...
  left_join(df_nonri, by = "choice_id")%>%
  left_join(df_ri, by = "choice_id")%>%
  left_join(df_park, by = "choice_id")%>%
  left_join(df_avail, by = "choice_id")

# this is differ from model 2 and 3 as it discriminate species risk and no risk
write_csv(df_apollo, "data/processed/ApolloData_nalt500_MNL_basic_mc+onethird-wagerate.csv")
