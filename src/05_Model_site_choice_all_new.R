...
# Title: model 5                                                               #
# Date: 9/29/2021                                                              #
# Description: Model site choice - include all birding hotspot                 #
################################################################################
...
df_travel_costs = readRDS("data/processed/df_travel_osrm_hotspots.rds")
...
# literature                                                                   #
#------------------------------------------------------------------------------#
# original analysis use opp_time 1/2, sensitivity analysis done using 1/2 and 
# full wage rate. To repeat sensitivity analysis change opp_time = 1/3 and 1
yearly_hours <- 2040
opp_time <- 1/2
vehicle_cost <- 0.3 

df_travel_costs <- df_travel_costs %>%
...
#df_travel_costs2 = read_csv("./data/processed/ab-ebd-travel-costs.csv")
...
df_park <- read_csv("./data/processed/nat_parks.csv") %>%
  left_join(read_csv("./data/processed/prov_parks.csv")) %>%
  left_join(read_csv("./data/processed/other_parks.csv")) %>%
  left_join(read_csv("./data/processed/out_parks.csv"))
...
df_trip <- df_pt_rel %>%
  ...
  left_join(df_travel_costs, by = c("postal_code", "locality_id")) %>%
...
if(consider_time == 1){ #  by time
  ...
  df_consideration_sets <- df_consideration_sets %>%
    filter(km <= travel_cutoff_hi & km >= travel_cutoff_lo) %>%
    select(observer_id, locality_id)
...
df_sites_per_obs2 = df_modeling %>%
  ...
  tally()  
# availability of number of hotpots in choice set
df <- ungroup(df_sites_per_obs2)
df <- select(df,observer_id,n)
df <- unique(df,incomparables = FALSE)
mean(df$n)
#----------------------------------------------------
...
library(priceR)
df4 = df3%>%
  mutate(adj_cost_total = cost_total)#adjust_for_inflation(cost_total,year, "CA", to_date = 2015))
# add back adj_cost to df_modeing
#since priceR taking time save df4 as a csv then no need to run above (priceR) 
# always
write.csv(df4,"data/processed/adjusted_cost_1_2_wagerate.csv")
#-------------------------------------------------------------------------------
# import adjusted cost folder
df_adjusted_cost <- read.csv("data/processed/adjusted_cost_1_2_wagerate.csv")
...
# save df_modeling
#write.csv(df_modeling,"data/processed/df_modeling.csv" )
...
#write_csv(df_apollo, "data/processed/ApolloData_nalt500_new.csv")
saveRDS(df_apollo, "data/processed/ApolloData_nalt500_new.RDS")
